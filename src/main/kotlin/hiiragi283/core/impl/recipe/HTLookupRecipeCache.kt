...
class HTLookupRecipeCache<INPUT : RecipeInput, RECIPE : Any>(
    ...
    override fun getFirstRecipe(input: INPUT, level: Level): RECIPE? {
        val holder: HTRecipeHolder<RECIPE> = lastRecipe ?: lookup.findFirst(level) { predicate(it, input, level) } ?: run {
            lastRecipe = null
    ...
    override fun serialize(output: HTValueOutput) {
        output.write("last_recipe", ResourceLocation.CODEC, lastRecipe?.id)
    }

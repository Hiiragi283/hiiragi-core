...
class HTThrownCaptureEgg : ThrowableItemProjectile {
    ...
    constructor(level: Level, shooter: LivingEntity) : super(HCEntityTypes.ELDRITCH_EGG.get(), shooter, level)

    constructor(level: Level, x: Double, y: Double, z: Double) : super(
        HCEntityTypes.ELDRITCH_EGG.get(),
        x,
        y,
        z,
        level,
    ...
    override fun onHitEntity(result: EntityHitResult) {
        ...
        if (!level().isClientSide) {
            ...
            // クリックしたモブのスポーンエッグを取得する
            val spawnEgg: ItemStack = getCapturedStack(target)
    ...
    override fun getDefaultItem(): Item = HCItems.ELDRITCH_EGG.get()
}

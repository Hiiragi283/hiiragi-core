...
import hiiragi283.core.api.inventory.container.type.HTItemMenuType
import hiiragi283.core.api.inventory.container.type.HTMenuTypeWithContext
...
/**
 * [MenuType]向けの[HTHolderLike]の実装クラスです。
 * @param MENU [AbstractContainerMenu]を継承したクラス
 * @param TYPE [MenuType]を継承したクラス
 ...
 */
sealed class HTDeferredMenuType<MENU : AbstractContainerMenu, TYPE : MenuType<MENU>>(private val key: ResourceKey<MenuType<*>>) :
    HTHolderLike<MenuType<*>, TYPE> {
    constructor(id: ResourceLocation) : this(Registries.MENU.createKey(id))

    final override fun getResourceKey(): ResourceKey<MenuType<*>> = key

    @Suppress("UNCHECKED_CAST")
    override fun get(): TYPE = BuiltInRegistries.MENU.getOrThrow(key) as TYPE
    ...
    class OnHand<MENU : AbstractContainerMenu> : HTDeferredMenuType<MENU, HTItemMenuType<MENU>> {
         ...
         * @return [get]の戻り値が[ItemStack]を継承していない場合は`null`
         */
        ...
        fun openMenu(player: Player, hand: InteractionHand?, stack: ItemStack): InteractionResult = when {
            stack.isEmpty -> InteractionResult.FAIL

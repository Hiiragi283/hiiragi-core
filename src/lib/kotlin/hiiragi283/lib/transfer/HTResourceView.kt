 ...
 * @see HTResourceSlot
 */
interface HTResourceView<T : Resource> : HTAmountView {
    fun getResource(): T
    ...
    fun getFilledLevel(resource: T): Float {
        val capacity: Long = getCapacityAsLong(resource)
        if (capacity <= 0) return 0f
        val fixedAmount: Long = minOf(getAmountAsLong(), capacity)
        return fixedAmount.toFloat() / capacity
    }

package hiiragi283.lib.gui.factory

import hiiragi283.lib.gui.menu.HTWidgetContainerMenu
...
data class HTBlockWidgetHolderContext(val factory: Factory, val player: Player, val pos: BlockPos) :
    ...
    companion object {
        @JvmField
        val MENU_TYPE: MenuType<HTWidgetContainerMenu> = IMenuTypeExtension.create(HTBlockWidgetHolderContext::create)
        ...
        fun create(containerId: Int, inventory: Inventory, buffer: RegistryFriendlyByteBuf): HTWidgetContainerMenu {
            ...
            if (block is Factory) {
                val context: HTBlockWidgetHolderContext = block.createContext(player, pos)
                return HTWidgetContainerMenu(MENU_TYPE, containerId, inventory, context)
            } else {
    ...
    override fun createMenu(containerId: Int, playerInventory: Inventory, player: Player): HTWidgetContainerMenu = HTWidgetContainerMenu(MENU_TYPE, containerId, playerInventory, this)

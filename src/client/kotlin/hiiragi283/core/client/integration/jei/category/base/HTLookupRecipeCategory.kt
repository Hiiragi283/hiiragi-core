...
sealed class HTLookupRecipeCategory<RECIPE : Any, HOLDER : Any>(
    ...
    final override fun getIdentifier(recipe: HOLDER): Identifier = lookup.getId(recipe)
    ...
    abstract class Fake<RECIPE : Any>(guiHelper: IGuiHelper, recipeType: HTFakeRecipeViewerType<*, RECIPE>) :
        HTLookupRecipeCategory<RECIPE, FakeRecipeHolder<RECIPE>>(guiHelper, recipeType, recipeType) {
        private val codec: Codec<FakeRecipeHolder<RECIPE>> = ResourceKey.codec(Registries.RECIPE).comapFlatMap(
            { key: RecipeKey ->
                recipeType
                    .getAllRecipes(HTJeiPlugin.createLookupContext())
                    .firstOrNull { it.key == key }
                    ?.let { DataResult.success(it) }
                    ?: DataResult.error { "Could not find recipe for key: ${key.identifier()}" }
            },
            FakeRecipeHolder<RECIPE>::key,
        )

        final override fun getCodec(codecHelper: ICodecHelper, recipeManager: IRecipeManager): Codec<FakeRecipeHolder<RECIPE>> = codec
    }
}

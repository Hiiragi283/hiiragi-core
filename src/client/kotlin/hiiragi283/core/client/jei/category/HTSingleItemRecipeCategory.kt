...
import hiiragi283.core.client.jei.extension.HTSingleItemRecipeCE
...
private typealias RecipeCE = HTSingleItemRecipeCE<HTSingleItemRecipe>
...
class HTSingleItemRecipeCategory(guiHelper: IGuiHelper, recipeType: HTLookupRecipeViewerType<*, HTSingleItemRecipe>) :
    ...
    private val extensions: MutableMap<Class<out HTSingleItemRecipe>, HTSingleItemRecipeCE<*>> = hashMapOf()

    inline fun <reified RECIPE : HTSingleItemRecipe> addExtension(extension: HTSingleItemRecipeCE<RECIPE>) {
        this.addExtension(RECIPE::class.java, extension)
    }

    fun <RECIPE : HTSingleItemRecipe> addExtension(clazz: Class<RECIPE>, extension: HTSingleItemRecipeCE<RECIPE>) {
    ...
    override fun setupRecipe(builder: IRecipeLayoutBuilder, recipe: HTSingleItemRecipe, focuses: IFocusGroup) {
        val (recipe1: HTSingleItemRecipe, extension: RecipeCE) = getExtension<HTSingleItemRecipe>(recipe) ?: return
        // input
        extension.setInput(
            recipe1,
    ...
    override fun onDisplayedIngredientsUpdate(
        ...
        val (recipe1: HTSingleItemRecipe, extension: RecipeCE) = getExtension<HTSingleItemRecipe>(recipe.recipe) ?: return
    ...
    private fun <RECIPE : HTSingleItemRecipe> getExtension(recipe: HTSingleItemRecipe): Pair<RECIPE, HTSingleItemRecipeCE<RECIPE>>? {
        val extension: HTSingleItemRecipeCE<RECIPE> =
            (extensions[recipe::class.java] as? HTSingleItemRecipeCE<RECIPE>) ?: run {
                for ((clazz: Class<out HTSingleItemRecipe>, extension: HTSingleItemRecipeCE<*>) in extensions) {
                    if (clazz.isInstance(recipe)) {
                        return@run extension as? HTSingleItemRecipeCE<RECIPE>
                    }
                }
                null
            } ?: return null

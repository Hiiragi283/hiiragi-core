...
interface HTAmountViewCapability<HANDLER : Any, ITEM_HANDLER : HANDLER> : HTMultiCapability<HANDLER, ITEM_HANDLER> {
     ...
     * @return [HTAmountView]の[List]
     */
    fun getCapabilityViews(stack: ItemStack): List<HTAmountView> = getCapability(stack)?.let { apply(it, null) } ?: listOf()
    ...
    fun getCapabilityViews(resource: HTItemResourceType?): List<HTAmountView> = getCapability(resource)?.let { apply(it, null) } ?: listOf()

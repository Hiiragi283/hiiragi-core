...
fun interface HTTagBuilder<R : Any> : Consumer<TagEntry> {
     ...
     */
    fun add(key: ResourceKey<R>, type: HTTagDependType = HTTagDependType.REQUIRED): HTTagBuilder<R> = add(key.location(), type)
     ...
     * @param type このエントリの依存関係
     */
    private fun add(id: ResourceLocation, type: HTTagDependType = HTTagDependType.REQUIRED): HTTagBuilder<R> = apply {
        when (type) {
     ...
     * @param type このエントリの依存関係
     */
    private fun addTag(id: ResourceLocation, type: HTTagDependType = HTTagDependType.REQUIRED): HTTagBuilder<R> = apply {
        when (type) {

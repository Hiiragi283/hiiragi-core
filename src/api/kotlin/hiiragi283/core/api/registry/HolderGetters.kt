...
fun <T : Any> RegistryAccess.registryResult(key: RegistryKey<T>): HTTextResult<Registry<T>> = this.registry(key).map(Registry<T>::right).getOrElse { HTTextResult("Registry ${key.location()} not found") }

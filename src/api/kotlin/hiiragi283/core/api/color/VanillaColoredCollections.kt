...
data object VanillaColoredCollections {
    ...
    @JvmStatic
    private inline fun <T> create(path: String, transform: (ResourceLocation) -> T): HTColoredCollection<T> = HTColoredCollection { transform(vanillaId(path.replace("%s", it.serializedName))) }
}
